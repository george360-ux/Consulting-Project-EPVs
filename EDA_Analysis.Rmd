...
fit_mixed_models <- function(df, property_filter) {
  
  df_prop <- df %>% filter(property == property_filter)
  
  # Mixed Model 1: Frontal only
  m_front <- lmer(
    OpticalProperty ~ Groups * factor(distance) + (1 | subID),
    data = df_prop %>% filter(Region == "front")
  )
  
  # Mixed Model 2: Occipital only
  m_occip <- lmer(
    OpticalProperty ~ Groups * factor(distance) + (1 | subID),
    data = df_prop %>% filter(Region == "occipital")
  )
  
  # Mixed Model 3: Combined regions (nested structure)
  m_combined <- lmer(
    OpticalProperty ~ Groups * factor(distance) * Region +
      (1 | subID) + (1 | subID:Region),
    data = df_prop
  )
...
# Run models
mixed_scatter <- fit_mixed_models(main_data, "scattering")
mixed_retard  <- fit_mixed_models(main_data, "retardance")

make_mixed_table <- function(model, title){
...
# Example:
make_mixed_table(mixed_scatter$frontal,
                 "Mixed-Effects Model — Scattering (Frontal)")
...
plot_qq(mixed_scatter$frontal, "QQ: Scattering (Frontal)")
plot_qq(mixed_scatter$occipital, "QQ: Scattering (Occipital)")
plot_qq(mixed_scatter$combined, "QQ: Scattering (Combined)")

plot_qq(mixed_retard$frontal, "QQ: Retardance (Frontal)")
plot_qq(mixed_retard$occipital, "QQ: Retardance (Occipital)")
plot_qq(mixed_retard$combined, "QQ: Retardance (Combined)")


```

```{r}
summary(mixed_scatter$frontal)
summary(mixed_scatter$occipital)
summary(mixed_scatter$combined)

summary(mixed_retard$frontal)
summary(mixed_retard$occipital)
summary(mixed_retard$combined)
